...
import { AlertCircle, Loader2, Play, Image as ImageIcon, Zap, Eye, Target } from 'lucide-react';
        ...
        <div className="flex flex-col h-full bg-background">
            ...
            <div className="flex items-center justify-between h-14 px-6 border-b border-border sticky top-0 z-10 bg-background/80 backdrop-blur-sm">
                ...
                <div className="flex items-center gap-4">
                    {/* View Controls */}
                    <div className="flex items-center bg-surface rounded-md border border-border p-0.5">
                        <button
                            onClick={() => setShowOverlay(!showOverlay)}
                            className={`p-1.5 rounded text-xs flex items-center gap-1.5 transition-colors ${showOverlay ? 'bg-primary/10 text-primary' : 'text-textMuted hover:text-textMain'}`}
                        ...
                        <button
                            ...
                            className={`px-2 py-1.5 rounded text-xs flex items-center gap-1.5 transition-colors ${focusMain ? 'bg-primary/10 text-primary font-medium' : 'text-textMuted hover:text-textMain'}`}
                            title="Focus Main Bike"
                        >
                            <Target size={14} />
                            {focusMain ? 'Main Only' : 'All Bikes'}
                        </button>
                    </div>
                    ...
                    <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded border transition-all shadow-sm ${isProcessing || completedCount === items.length
            ...
            <div className="flex-1 overflow-y-auto">
                {items.map((item, idx) => {
                    ...
                    // Filter detections for display based on focus mode
                    const displayDetections = focusMain
                        ? item.detections.filter(d => d.type === 'Main')
                        : item.detections;
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="col-span-4 flex items-center gap-3 overflow-hidden">
                                <div className="w-24 h-16 rounded bg-surface border border-border overflow-hidden shrink-0 group-hover:border-textMuted transition-colors relative">
                                    <ImageOverlay
                            ...
                            <div className="col-span-7">
                                {item.status === ProcessingStatus.ERROR ? (
                                    ...
                                    <div className="flex flex-wrap gap-2 items-center">
                                        {displayDetections.map((d, i) => (
                                            <span key={i} className="inline-flex items-center gap-1.5 px-1.5 py-0.5 rounded text-[11px] border border-border bg-surface text-textMuted/90 shadow-sm">
                                                ...
                                                />
                                                <span className="font-medium text-textMain"><span className="text-textMuted font-normal">Frame:</span> {d.frameColor}</span>
                                                <span className="text-textMuted opacity-50">/</span>
                                                <span className=""><span className="text-textMuted font-normal">Fender:</span> {d.fenderColor === 'Unknown' ? 'No Fender' : d.fenderColor}</span>
                                    ...
                                    <span className="text-textMuted/30 text-xs font-normal">
                                        {item.status === ProcessingStatus.COMPLETED ? (focusMain && item.detections.length > 0 ? 'No main bike detected' : 'No bikes detected') : 'Waiting for analysis...'}
                                    </span>
                                )}
                            </div>
                        </div>
